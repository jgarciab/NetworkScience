...
```{r}
netdata<-read.csv("./data/day5_socialdynamics_Networkdata.csv", header = TRUE)
...
Check out the network and find the weak ties and strong ties. You can zoom in and zoom out to check the links; by clicking a node, its id will be shown on the small window of left hand side; and its nearby nodes will be highlighted. Recall the definitions of weak ties and strong ties in the lecture.
...
**After you build the model, apply it to the Network1, and record the number of newly infected people by day (i.e., newly confirmed cases by day) for further analysis.**
...
Based upon Network 1, **delete 3 WEAK ties** that you have identified to generate a new network (Network 2). **Apply the IC model to the Network 2**, and record the number of newly infected people by day for further analysis. \### 3.1 Reconstruct the network by deleting 3 weak ties
...
Network2<-Network1 %>%
  delete_edges("18|44") %>%   #!!!Change to your own numbers
  delete_edges("36|32") %>%   #!!!Change to your own numbers
  delete_edges("49|48")       #!!!Change to your own numbers
```
...
Based upon Network 1, **delete 3 STRONG ties** that you have identified to generate a new network (Network 3). **Apply the IC model to the Network 3**, and record the number of newly infected people by day for further analysis. \### 4.1 Reconstruct the network by deleting 3 strong ties

```{r}
#Change the numbers to the ties that you want to delete,
#e.g., if you want to delete the tie between 22 and 46, change "1|2" to "22|46"
Network3<-Network1 %>%
  delete_edges("54|53") %>%   #!!!Change to your own numbers
  delete_edges("50|51") %>%   #!!!Change to your own numbers
...
**Generate a plot** (Plot_p0) to compare the daily infected percentages of Network 1, Network 2 and Network 3. What do you observe? Considering the fact that the amount of edges we deleted in Network 2 and 3 account for around 2% of the total edges in Network 1, what is the implication of your observation?
...
```{r}
# What do you observe? Considering the fact that the amount of edges we deleted in Network 2 and 3 account for around 2% of the total edges in Network 1, what is the implication of your observation? Answer:
```

## Question 6

Try to explain the your observation in Question 5 in more details as a network scientist-- by looking at the change of network structure. To do so, we suggest you to check the following network attributes of Network 1, Network 2 and Network 3:
...
cat('transitivity removing strong ties:', transitivity(Network2,type="global"), '\n' )
...
In the lecture, we did a small survey on your 'threshold' to restrict your beef consumption to once a week or lower. Here, let's **build a threshold model** to simulate the spread of "once-a-week-beef" campaign in Network 1. Below is the model description:
...
-   At Day 1, a group of enthusiastic people (Node ID={1, 2, 5, 11}) in the network decided to take action and adopt their own behaviors (change the status value to 1);

-   At the following days, for nodes who haven't adopted, they will check the status of their neighbors to decide to adopt or not:

    -   For example, for node i, it has a predefined threshold of $Ɵ_i$. And node i has $N_i$ number of neighbours in the whole network.

    -   Among all the neighbours of Node i, if more than $Ni\cdot Ɵi$ of them have adopted, node i will also adopt and change the status value to 1.
...
### Functions to build the threshold model
...
Apply the threshold model by activating **only one node** (id = 5).
Recall the diffusion of virus we simulated in the morning: a network-wide contagion has been observed with only one node (id=5) being activated at Day 0, for various values of p. Use the theory of "width of bridge" in complex contagion, **explain why a network-wide contagion is not happening here**.
...
```{r}
igraph::degree(Network1) #degree by nodes
...
```{r}
seedHD <- order(igraph::degree(Network1), decreasing=TRUE)[1:4]
```
