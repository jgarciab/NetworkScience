...
# Package names
packages <- c('igraph', 'visNetwork',"htmlwidgets")
...
IC<-function(node_seed,network,Pprob, days){
...
days <- 28 # choose number of days

result1<- replicate(100, IC(5,Network1,0.35, days), simplify=FALSE)  #run 100 times since each IC model run has it own randomness 
...
Network2<-Network1 %>%
  delete_edges("22|46") %>%   #!!!Change to your own numbers
  delete_edges("32|36") %>%   #!!!Change to your own numbers
  delete_edges("15|42")       #!!!Change to your own numbers
```

### 3.2 Apply the IC model to Network 2

```{r}
result2<- replicate(100, IC(5,Network2,0.35, days), simplify=FALSE)  #run 100 times since each IC model run has it own randomness 
...
#e.g., if you want to delete the tie between 22 and 46, change "1|2" to "22|46"
Network3<-Network1 %>%
  delete_edges("51|54") %>%   #!!!Change to your own numbers
  delete_edges("38|43") %>%   #!!!Change to your own numbers
  delete_edges("16|5")       #!!!Change to your own numbers
...
```{r}
result3<- replicate(100, IC(5,Network3,0.35, days), simplify=FALSE)  #run 100 times since each IC model run has it own randomness 
...
```{r}
SumResult<-data.frame(matrix(nrow=days*3,ncol=3))
names(SumResult)=c("Day","Network","Infected")
SumResult[,1]=rep(c(1:days),3)
SumResult[,2]=c(rep("Original network",days),rep("Remove weak ties",days),rep("Remove strong ties",days))
...
result1_p1<- replicate(100, IC(5,Network1,p1,days), simplify=FALSE)  
result1_p1<-do.call(rbind, result1_p1)
result1_p1_forplot<-colMeans(result1_p1)

result2_p1<- replicate(100, IC(5,Network2,p1,days), simplify=FALSE)  
result2_p1<-do.call(rbind, result2_p1)
result2_p1_forplot<-colMeans(result2_p1)

result3_p1<- replicate(100, IC(5,Network3,p1,days), simplify=FALSE)  #run 100 times since each IC model run has it own randomness 
result3_p1<-do.call(rbind, result3_p1)
result3_p1_forplot<-colMeans(result3_p1)

SumResult_p1<-data.frame(matrix(nrow=days*3,ncol=3))
names(SumResult_p1)=c("Day","Network","Infected")
SumResult_p1[,1]=rep(c(1:days),3)
SumResult_p1[,2]=c(rep("Original network",days),rep("Remove weak ties",days),rep("Remove strong ties",days))
...
result1_p2<- replicate(100, IC(5,Network1,p2, days), simplify=FALSE)  
result1_p2<-do.call(rbind, result1_p2)
result1_p2_forplot<-colMeans(result1_p2)

result2_p2<- replicate(100, IC(5,Network2,p2,days), simplify=FALSE)  
result2_p2<-do.call(rbind, result2_p2)
result2_p2_forplot<-colMeans(result2_p2)

result3_p2<- replicate(100, IC(5,Network3,p2,days), simplify=FALSE)  #run 100 times since each IC model run has it own randomness 
result3_p2<-do.call(rbind, result3_p2)
result3_p2_forplot<-colMeans(result3_p2)

SumResult_p2<-data.frame(matrix(nrow=days*3,ncol=3))
names(SumResult_p2)=c("Day","Network","Infected")
SumResult_p2[,1]=rep(c(1:days),3)
SumResult_p2[,2]=c(rep("Original network",days),rep("Remove weak ties",days),rep("Remove strong ties",days))
